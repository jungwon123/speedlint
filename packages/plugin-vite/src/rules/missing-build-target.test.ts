...
function makeContext(files: Record<string, string>): RuleContext {
	...
	return { project: { root: "/test", bundler: "vite", packageJson: {} } as ProjectContext, files: fileMap, getAST: () => ({}), getDependencyGraph: () => ({ nodes: new Map() }), getConfig: () => undefined, report: () => {} };
}

describe("vite/missing-build-target", () => {
	it("should detect missing build target", () => {
		const ctx = makeContext({ "vite.config.ts": "export default defineConfig({ plugins: [react()] })" });
		expect(missingBuildTarget.detect(ctx)).toHaveLength(1);
	});
	it("should not flag if target is set", () => {
		const ctx = makeContext({ "vite.config.ts": "export default defineConfig({ build: { target: 'es2022' } })" });
